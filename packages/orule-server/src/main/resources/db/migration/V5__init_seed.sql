-- V5: 种子数据
-- RFC-0032 §3.7 重构版：
--   1) CustomerTier 升格为 object_type(kind=ENUM)，values 存于 enum_values
--   2) attribute_type 用 3 列结构（data_type + sub_data_type_program_code + sub2）
--   3) code → program_code
...
-- Customer attributes：tier 字段通过 programCode 引用 CustomerTier (kind=ENUM)
...
-- Order attributes：list/map 通过 sub 列表达元素类型
